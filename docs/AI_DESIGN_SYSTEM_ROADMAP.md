...
- local semantic cue families now also separate editor/canvas, catalog/store, profile/community, map/navigation, and media/player screenshots from those broader buckets, reducing more note/filename-driven non-OCR false positives without bundling built-in model dependencies
...
- recipe-specific composition/background reactions now respond more explicitly to onboarding, paywall, settings, chat, reporting, workflow, discovery, editor, profile, catalog, map, and media-style screens instead of only generic frame/crop signals
- panoramic planning/materialization now also emits tool-ribbon, profile-spotlight, browse-strip, route-arc, and playback-marquee treatments plus background accents when those local cue families are present, so the strip reacts more clearly than generic proof/decorative systems alone
