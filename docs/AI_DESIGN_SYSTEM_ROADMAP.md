...
- preview review now also supports persisted deterministic opener / intensify / resolve beat overrides for layout archetype and support rhythm, and reviewed rebuilds reuse those saved controls so one panoramic concept can diverge internally instead of staying locked to concept-level rhythm only
...
- raster-family structural scoring now also helps more distinctive non-OCR profile/document/editor/catalog/map/media/capture layouts plus support/help-style list flows resolve into the right semantic family without OCR text, gives help-list/support-card structures stronger reviewed support fallback behavior, surfaces checkout-style summary/price-rail rows as stronger reviewed commerce contenders while keeping quieter onboarding/control rails more conservative, gives secure-access screens more weight when focused auth-gate structure aligns with explicit security cues, lets raster-only reward/loyalty structure contribute directly to reward-family resolution, broadens deterministic support/help, catalog, commerce, reward, and secure-access text labels, and adds stronger ambiguity guards so warm browse/profile layouts do not overread as reward and checkout-like settings/account rails do not overread as commerce/security as often
- raster-family structural scoring now also lets stronger non-OCR checkout cards resolve directly to commerce when repeated price rails and a clear handoff CTA line up, so checkout-lane planning reactions can trigger without OCR while settings/account ambiguity guards remain conservative
...
- review-to-generation loop now also supports a faster local refresh path in preview, so reviewed screenshot-family changes can rebuild autopilot concepts, rerender previews, and rescore the refreshed session in one pass
- review-to-generation loop now also supports reviewed comparison branches, so family-review changes can preserve the current concepts, rerender only the new reviewed branches, and rescore the session for quicker side-by-side comparison
...
- beat-level panoramic review controls now also let opener / intensify / resolve beats override layout archetype and support rhythm independently during reviewed rebuilds, giving the preview loop a faster way to break repeated family pacing without adding model-dependent generation
...
- local cue-family understanding now also detects editor/canvas, catalog/store, profile/community, activity/feed, document/review, map/navigation, media/player, capture/scan, schedule/calendar, commerce/checkout, secure-access, support/help, and rewards/loyalty screenshots so those flows stop collapsing into generic settings/detail/reporting/editor/media/paywall reads as often; commerce/checkout and secure-access cue coverage now recognizes more subtotal/shipping/total and magic-link/one-time-code/trusted-device labels, deterministic support/help and catalog/reward/profile text cues are broader, secure-access scoring now responds more strongly when explicit auth cues line up with focused centered-panel/CTA structure, and settings/account evidence plus quiet control-rail guards now block more checkout/security/reward overreads when explicit family cues are absent
