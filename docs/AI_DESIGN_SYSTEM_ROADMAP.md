...
- preview review surface: sessions now carry per-concept copy-slot assignments and the preview UI exposes the selected copy plan per concept
- manual refinement path: users can now approve the recommended concept and persist manual edits back into the active variant session without depending on an AI agent for every change
