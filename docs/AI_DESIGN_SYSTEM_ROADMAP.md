...
- local semantic cue families now also separate editor/canvas, catalog/store, profile/community, map/navigation, media/player, capture/scan, and schedule/calendar screenshots from those broader buckets, reducing more note/filename-driven non-OCR false positives without bundling built-in model dependencies
...
- materialized configs now consume that planning metadata to drive frameless support treatment, loupe anchoring, support-card crop behavior, safer text offsets, palette-led backgrounds, deeper device-frame treatment, and role-aware composition/background reactions for onboarding/paywall/settings/chat/reporting/workflow/discovery/editor/profile/catalog/map/media/capture/schedule-style screens
...
- local cue-family understanding now also detects editor/canvas, catalog/store, profile/community, map/navigation, media/player, capture/scan, and schedule/calendar screenshots so those flows stop collapsing into generic settings/detail/reporting/editor/media reads as often
...
- recipe-specific composition/background reactions now respond more explicitly to onboarding, paywall, settings, chat, reporting, workflow, discovery, editor, profile, catalog, map, media, capture, and schedule-style screens instead of only generic frame/crop signals
- panoramic planning/materialization now also emits tool-ribbon, profile-spotlight, browse-strip, route-arc, playback-marquee, capture-focus, and timeline-band treatments plus background accents when those local cue families are present, so the strip reacts more clearly than generic proof/decorative systems alone
