...
- [x] Add better role detection for:
  ...
  Status: OCR/layout semantics now improve these cases when text enrichment is available, raster/non-OCR deterministic logic now covers the same families plus workflow/discovery and local editor/profile/catalog/activity/document/map/media/capture/schedule/commerce/security/support/reward cue families, and ambiguity guards now cut more settings/reporting/editor/media/paywall false positives; broader scene-graph understanding is still incomplete.
