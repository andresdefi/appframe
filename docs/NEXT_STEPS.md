...
- screenshot analysis now also uses raster-family structural scoring for more distinctive non-OCR document/editor/catalog/map/media/capture-like layouts and adds stronger paywall-shape ambiguity rejection so weak rewards/commerce filename cues do not override generic premium CTA structure as often
...
- preview UI session analysis now also lets users override per-frame panoramic rhythm, continuity motif, and support system, persists those overrides with the session, and reapplies them during reviewed-session rebuilds so art-direction tweaks are not lost between review and rematerialization
- preview UI session analysis now also adds concept-level panoramic art-direction controls for rhythm presets, bulk continuity/support overrides, reset-to-auto behavior, and visible pacing summaries so editors can steer a whole strip faster than per-frame dropdowns alone
...
- [variant-session-tools.test.ts](/Users/bastianvidela/appframe/packages/mcp-server/src/tools/variant-session-tools.test.ts)
...
pnpm vitest run packages/mcp-server/src/tools/design-planning.test.ts \
  ...
  packages/web-preview/src/client/store.test.ts
```
...
- [x] Add better role detection for:
  ...
  Status: OCR/layout semantics now improve these cases when text enrichment is available, raster/non-OCR deterministic logic now covers the same families plus workflow/discovery and local editor/profile/catalog/activity/document/map/media/capture/schedule/commerce/security/support/reward cue families, raster-family scoring now helps some structurally distinctive non-OCR screens resolve into the right semantic family even with generic filenames, and ambiguity guards now cut more settings/reporting/editor/media/paywall false positives; broader scene-graph understanding is still incomplete.
...
- [x] Allow reviewed screenshot semantic-family metadata to drive replanning without re-inferring families.
  Status: `buildVariantSetPlanFromAnalysis` now reuses persisted/reviewed `ScreenshotAnalysis[]`, and `appframe_plan_variant_set` accepts reviewed `screenshotAnalysisJson` so session review edits can feed forward into new plans deterministically.
...
- [x] Preserve explicit panoramic rhythm / continuity / support-system review overrides through autopilot session rebuilds.
  Status: Preview Session Analysis now persists per-frame panoramic art-direction overrides and the shared rebuild path reapplies them before rematerialization.
...
- [x] Add first-pass preview controls for panoramic support-system / continuity / rhythm art direction.
  Status: Session Analysis now exposes per-frame rhythm-role, continuity-motif, and support-system selectors that save into the session and feed the next reviewed rebuild.
...
1. keep improving screenshot understanding so plans react more confidently to the broader local family set now that activity/feed and document/review cues are also covered
2. keep expanding screenshot-understanding quality beyond the current raster-layout heuristics and local note/filename cues so non-OCR analysis keeps improving without regressing into false positives, especially for scene families beyond the current workflow/discovery/editor/profile/catalog/activity/document/map/media/capture/schedule/commerce/security/support/reward coverage
...
That is the next quality step now that reviewed screenshot-family metadata and first-pass panoramic art-direction overrides can already feed plan/materialization refreshes through both MCP and direct preview-side rebuild paths.

## Suggested Concrete Next Task Prompt

Use this to start a future thread:

> Continue AppFrame autopilot work. Read [NEXT_STEPS.md](/Users/bastianvidela/appframe/docs/NEXT_STEPS.md) and [AI_DESIGN_SYSTEM_ROADMAP.md](/Users/bastianvidela/appframe/docs/AI_DESIGN_SYSTEM_ROADMAP.md). Build the next screenshot-intelligence slice after the current local cue-family pass: reduce remaining non-OCR false positives, broaden semantic coverage beyond the current onboarding/paywall/settings/chat/dashboard/workflow/discovery/editor/profile/catalog/activity/document/map/media/capture/schedule/commerce/security/support/reward heuristics, and keep expanding recipe-specific composition/background mapping plus panoramic pacing for both individual and panoramic concepts.
...
- Preview sessions now persist manual screenshot semantic-family review metadata, and both MCP/session tooling plus the preview-side rebuild action can already reuse current review state for replanning/materialization; per-frame panoramic overrides plus first-pass concept-level rhythm/support/continuity bulk controls are now included, but deeper recipe-inspector and continuity-editor tooling is still open.
