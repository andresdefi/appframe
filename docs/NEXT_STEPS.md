...
- screenshot analysis now also uses broader raster-family structural scoring for more distinctive non-OCR profile/document/editor/catalog/map/media/capture-like layouts, now gives support/help-style list flows stronger raster corroboration, broadens deterministic support/help, profile, catalog, commerce, reward, and generic-control text cues, surfaces checkout-style summary/price-rail rows as stronger reviewed commerce contenders, keeps quiet onboarding-style control rails and generic settings/account surfaces from over-triggering secure-access or commerce treatment as often, and stays more conservative when warm browse/profile layouts could otherwise overread as reward without OCR text
- screenshot analysis now also lets stronger raster-only checkout structure resolve directly to commerce when repeated price rails and a clear handoff CTA are present, so local planning can trigger checkout-lane reactions without OCR while existing settings/account ambiguity guards stay in place
- screenshot analysis now also lets raster-only reward/loyalty structure contribute directly to semantic-family resolution instead of depending mostly on text cues, so non-OCR member-perk / cashback / loyalty-wallet screens can trigger reward planning reactions while settings/paywall ambiguity stays conservative
...
- [x] Add better role detection for:
  ...
  Status: OCR/layout semantics now improve these cases when text enrichment is available, raster/non-OCR deterministic logic now covers the same families plus workflow/discovery and local editor/profile/catalog/activity/document/map/media/capture/schedule/commerce/security/support/reward cue families, raster-family scoring now helps more structurally distinctive non-OCR profile/document/editor/catalog/map/media/capture screens plus support/help-style list flows resolve into the right semantic family even with generic filenames, support-style list/case layouts can now upgrade into reviewed support more directly, strong checkout-style summary/price-rail/CTA structure can now resolve directly into commerce while weaker checkout-like rails still stay reviewed, warm browse/profile structures now avoid some reward overreads, deterministic text cue coverage now catches broader support/help, catalog, commerce, reward, and secure-access labels without a bundled model dependency, and ambiguity guards still cut more settings/reporting/profile/reward/commerce/media/paywall/security false positives including settings/account screens with checkout-like right-rail structure; broader scene-graph understanding is still incomplete.
...
- [x] Add screenshot semantic-family rationale/ambiguity diagnostics and bulk review affordances in preview sessions.
  Status: preview session analysis now shows semantic-family reasoning, competing local family reads, review-needed states, and bulk actions to mark flagged screenshots generic or reset reviewed overrides.
