...
function makeConfig(
  ...
  options?: {
    headlines?: string[];
    panoramicElements?: NonNullable<AppframeConfig['panoramic']>['elements'];
  },
): AppframeConfig {
    ...
    ...(mode === 'panoramic'
      ? {
          ...
          panoramic: {
            ...
            elements: options?.panoramicElements ?? [
              {
                type: 'text' as const,
...
describe('preview scoring', () => {
  ...
  it('penalizes repeated panoramic support-group rhythm even without preview artifacts', () => {
    const repeatedSupport = makeConfig('panoramic', 'editorial', {
      panoramicElements: [
        {
          type: 'text',
      ...
      ] as NonNullable<AppframeConfig['panoramic']>['elements'],
    });

    const variedSupport = makeConfig('panoramic', 'editorial', {
      panoramicElements: [
        {
          type: 'text',
          content: 'Stay on track',
        ...
        },
        {
          ...
          children: [
            ...
            },
            {
              ...
              height: 32,
              eyebrow: 'Lead',
              title: 'Proof stays clear',
              body: 'Different support shapes should score higher.',
              align: 'left',
              backgroundColor: '#FFFFFF',
              opacity: 0.96,
              borderColor: '#2563EB',
              ...
              titleSize: 4.8,
              bodySize: 2.8,
              shadow: { opacity: 0.1, blur: 10, offsetY: 4, color: '#0F172A' },
              z: 1,
            },
          ],
        },
    ...
    const result = scoreVariantSet([
      {
        id: 'concept-c',
        name: 'Repeated Support',
        config: repeatedSupport,
        previewCount: 0,
      },
      {
        id: 'concept-d',
        name: 'Varied Support',
        config: variedSupport,
        previewCount: 0,
      },
    ]);

    const repeated = result.scored.find((variant) => variant.id === 'concept-c')!;
    const varied = result.scored.find((variant) => variant.id === 'concept-d')!;

    expect(varied.score.breakdown.recipeSpecificity ?? 0).toBeGreaterThan(repeated.score.breakdown.recipeSpecificity ?? 0);
    expect(repeated.score.flags.some((flag) => flag.includes('support groups reuse the same child structure'))).toBe(true);
  });
