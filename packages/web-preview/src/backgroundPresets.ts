// Background preset catalog. Lives at the package root (not in src/client/)
// so both the client bundle and the server tsc build can import it. The
// client's utils/presets.ts re-exports from here for backwards compat, and
// the server serves the categorized catalog on /api/background-presets so
// the MCP can surface the same palette the UI shows.
