...
export function PanoramicPreview() {
  ...
  return (
    ...
    <div ref={areaRef} className="flex-1 flex flex-col overflow-hidden bg-bg relative">
      {/* Beta banner — panoramic mode has unreliable interactive
          controls (drag-to-reposition doesn't track live, some sliders
          don't patch live) in both backends. Surfaced here so users
          don't think they've broken anything. */}
      <div className="shrink-0 flex items-center gap-2 px-4 py-2 text-[11px] bg-amber-500/10 border-b border-amber-500/30 text-amber-200">
        <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-100 font-semibold text-[10px] uppercase tracking-wider">
          Beta
        </span>
        <span>
          Panoramic mode is a work in progress — some controls may not respond as expected.
        </span>
      </div>
